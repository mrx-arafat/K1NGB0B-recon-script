...
def check_tool(tool_name: str) -> bool:
    ...
    # Try different methods to check if tool exists
    try:
        # Method 1: Try running the tool with --help
    ...
    try:
        # Method 2: Try running the tool with --version
    ...
    try:
        # Method 3: Use which command (Unix/Linux)
    ...
    try:
        # Method 4: Use where command (Windows)
...
def run_basic_subdomain_discovery(domain: str, output_file: str, source_name: str) -> List[str]:
    """Basic subdomain discovery using common patterns when tools are missing."""
    print(f"   🔄 Running basic discovery for {source_name}...")

    # Common subdomain patterns for VPS environments
    common_subdomains = [
        'www', 'mail', 'ftp', 'admin', 'api', 'app', 'dev', 'test', 'staging',
        'prod', 'production', 'beta', 'alpha', 'demo', 'blog', 'shop', 'store',
        'support', 'help', 'docs', 'portal', 'dashboard', 'panel', 'cpanel',
        'webmail', 'secure', 'ssl', 'vpn', 'remote', 'cloud', 'cdn', 'static',
        'assets', 'img', 'images', 'media', 'files', 'download', 'uploads',
        'backup', 'old', 'new', 'mobile', 'm', 'wap', 'forum', 'chat', 'news'
    ]
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(f"# Basic subdomain discovery results for {domain}\n")
        f.write(f"# Source: {source_name}\n")
        f.write(f"# Found: {len(discovered)} subdomains\n\n")
        for subdomain in discovered:
            f.write(f"{subdomain}\n")

    print(f"   ✅ Basic discovery found {len(discovered)} subdomains")
    return discovered
...
def run_assetfinder(domain: str, output_file: str) -> List[str]:
    ...
    if not check_tool('assetfinder'):
        print("   ⚠️  AssetFinder not found, using built-in discovery...")
        return run_basic_subdomain_discovery(domain, output_file, "assetfinder_fallback")
...
def run_subfinder(domain: str, output_file: str) -> List[str]:
    ...
    if not check_tool('subfinder'):
        print("   ⚠️  Subfinder not found, using built-in discovery...")
        return run_basic_subdomain_discovery(domain, output_file, "subfinder_fallback")
...
async def run_amass(domain: str, output_file: str) -> List[str]:
    ...
    if not check_tool('amass'):
        print("   ⚠️  Amass not found, using built-in discovery...")
        return run_basic_subdomain_discovery(domain, output_file, "amass_fallback")
...
def run_simple_reconnaissance(domain: str):
    """Simplified reconnaissance that works reliably on VPS environments."""
    print(f"🎯 Target: {domain}")
    print("=" * 60)

    # Create output directory
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = f"{domain.replace('.', '_')}_results_{timestamp}"

    directories = [
        "01_raw_discovery", "02_processed_data", "03_live_analysis",
        "04_technologies", "05_vulnerabilities", "06_port_scanning",
        "07_screenshots", "08_final_reports", "09_advanced_discovery",
        "10_manual_verification"
    ]

    for directory in directories:
        os.makedirs(f"{output_dir}/{directory}", exist_ok=True)

    print(f"📂 Created output structure: {output_dir}/")

    all_subdomains = set()

    # Phase 1: Basic Discovery
    print(f"\n🔍 Phase 1: Basic Subdomain Discovery")
    print("-" * 40)

    # Run tools with fallbacks
    print("🔧 Running AssetFinder...")
    assetfinder_results = run_assetfinder(domain, f"{output_dir}/01_raw_discovery/assetfinder.txt")
    all_subdomains.update(assetfinder_results)
    print(f"✅ AssetFinder: {len(assetfinder_results)} subdomains")

    print("🔧 Running Subfinder...")
    subfinder_results = run_subfinder(domain, f"{output_dir}/01_raw_discovery/subfinder.txt")
    all_subdomains.update(subfinder_results)
    print(f"✅ Subfinder: {len(subfinder_results)} subdomains")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Certificate Transparency: {len(ct_subdomains)} subdomains")

            # Save CT results
            with open(f"{output_dir}/01_raw_discovery/certificate_transparency.txt", 'w', encoding='utf-8') as f:
                for subdomain in sorted(ct_subdomains):
                    f.write(f"{subdomain}\n")
        else:
            print(f"⚠️  Certificate Transparency failed: HTTP {response.status_code}")
    except Exception as e:
        print(f"⚠️  Certificate Transparency error: {e}")
    ...
    # Save all subdomains
    all_subs_file = f"{output_dir}/02_processed_data/all_subdomains.txt"
    with open(all_subs_file, 'w', encoding='utf-8') as f:
        f.write(f"# All discovered subdomains for {domain}\n")
        f.write(f"# Generated: {datetime.now().isoformat()}\n")
        f.write(f"# Total: {len(cleaned_subdomains)} unique subdomains\n\n")
        for subdomain in sorted(cleaned_subdomains):
            f.write(f"{subdomain}\n")

    print(f"✅ Processed {len(cleaned_subdomains)} unique subdomains")

    # Identify critical subdomains
    critical_keywords = ['admin', 'api', 'app', 'staging', 'dev', 'test', 'prod', 'panel', 'dashboard']
    critical_subdomains = [sub for sub in cleaned_subdomains
                          if any(keyword in sub.lower() for keyword in critical_keywords)]

    if critical_subdomains:
        critical_file = f"{output_dir}/02_processed_data/critical_subdomains.txt"
        with open(critical_file, 'w', encoding='utf-8') as f:
            f.write(f"# Critical subdomains for {domain}\n")
            f.write(f"# Total: {len(critical_subdomains)} critical subdomains\n\n")
            for subdomain in sorted(critical_subdomains):
                f.write(f"{subdomain}\n")

        print(f"🎯 Identified {len(critical_subdomains)} critical subdomains")
    ...
    summary = {
        "target": domain,
        "timestamp": datetime.now().isoformat(),
        "total_subdomains": len(cleaned_subdomains),
        "critical_subdomains": len(critical_subdomains),
        "output_directory": output_dir
    }

    # Save JSON summary
    with open(f"{output_dir}/08_final_reports/summary.json", 'w', encoding='utf-8') as f:
        json.dump(summary, f, indent=2)

    # Create executive summary
    exec_summary = f"""🔥 K1NGB0B Domain Discovery - Executive Summary
Target: {domain}
Scan Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

📊 DISCOVERY RESULTS:
• Total Subdomains: {len(cleaned_subdomains)}
• Critical Subdomains: {len(critical_subdomains)}
• Output Directory: {output_dir}

📁 KEY FILES:
• All Subdomains: 02_processed_data/all_subdomains.txt
• Critical Subdomains: 02_processed_data/critical_subdomains.txt
• JSON Summary: 08_final_reports/summary.json

🎯 NEXT STEPS:
1. Review all discovered subdomains
2. Manually test critical subdomains
3. Install additional tools for enhanced scanning
4. Run vulnerability scans on live targets

Generated by K1NGB0B Domain Discovery v2.0
"""

    with open(f"{output_dir}/08_final_reports/executive_summary.txt", 'w', encoding='utf-8') as f:
        f.write(exec_summary)
    ...
    print("=" * 60)
    print(f"🎯 Target: {domain}")
    print(f"📈 Total subdomains discovered: {len(cleaned_subdomains)}")
    print(f"🎯 Critical subdomains found: {len(critical_subdomains)}")
    print(f"📁 Results saved to: {output_dir}/")

    if critical_subdomains:
        print(f"\n🎯 Critical Subdomains Found:")
        for subdomain in sorted(critical_subdomains)[:10]:
            print(f"   • {subdomain}")
        if len(critical_subdomains) > 10:
            print(f"   ... and {len(critical_subdomains) - 10} more")

    print(f"\n📊 Next Steps:")
    print(f"   📋 Review: {output_dir}/08_final_reports/executive_summary.txt")
    print(f"   🎯 Critical: {output_dir}/02_processed_data/critical_subdomains.txt")
    print(f"   📝 All Results: {output_dir}/02_processed_data/all_subdomains.txt")
    ...
    try:
        ...
        if len(sys.argv) > 1:
            if sys.argv[1] in ['--help', '-h']:
                print("\n📖 Usage:")
                print("   python domain_discovery.py <domain>")
                print("   python domain_discovery.py tesla.com")
                print("\n🔧 Features:")
                print("   • 15+ Advanced Discovery Techniques")
                print("   • Professional Output Structure")
                print("   • Works with or without external tools")
                print("   • Certificate Transparency Mining")
        ...
        # Check dependencies (but don't stop if missing)
        print("\n🔍 Checking dependencies...")
        missing_deps, optional_missing = check_dependencies()

        if missing_deps:
            print(f"\n⚠️  Missing some tools (will use available alternatives):")
            for dep in missing_deps:
                print(f"   • {dep}")
            print(f"\n💡 To install missing tools, run:")
            print(f"   chmod +x quick_install.sh && ./quick_install.sh")
            print(f"   # OR")
        ...
        # Run simplified reconnaissance
        print(f"\n🚀 Starting reconnaissance...")

        try:
            run_simple_reconnaissance(domain)
        except Exception as e:
